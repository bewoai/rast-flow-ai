...
## 📋 [PROMPT 2] — AutoCut Puanlamalı Tekrar Avcısı, Alt+Z Geri Alma & Whisper Parçalama

**Hedef**: AutoCut'ın özel kayan pencere tekrar eşleme algoritmasını entegre etmek, görsel tekrar kartları oluşturmak, `Alt + Z` kısayollu Undo motorunu yazmak ve uzun sesler için Whisper dilimleme eklemek.

```markdown
Rast Flow AI projemize AutoCut'ın matematiksel tekrar algılama motorunu, "Alt + Z" ile çakışmasız geri alma (Undo) motorunu ve büyük dosyalar için otomatik parçalama eklemek istiyoruz.

Lütfen kod üzerinde şu geliştirmeleri yap:

1. AutoCut Algoritması ile Tekrar Avcısı (app.js - findRepeats):
- `findRepeats(method)` fonksiyonunu AutoCut'ın sliding-window (kayan pencere) sequence-matcher algoritmasına göre güncelle.
...
Rast Flow AI projemize, AutoCut'ın OpenAI entegrasyonundan ilham alan ve transkripti anlamlı bir şekilde temizleyip hatalı çekimleri (multi-take) eleyen gelişmiş bir "AI Editor" sekmesi ve motoru eklemek istiyoruz.
